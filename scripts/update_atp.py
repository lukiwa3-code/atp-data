...
def save_matches_safely(path: Path, new_payload: Dict[str, Any]) -> Dict[str, Any]:
    """Nie nadpisuj dobrych danych zerem.

    Jeżeli ATP/Cloudflare zwróci pustą stronę albo parser znajdzie 0 meczów,
    zostawiamy stare matches.json bez żadnego dopisywania flag.
    Dzięki temu przy blokadzie ATP nie powstają puste commity i nie znikają wyniki.
    """
...
def fetch_tournament_draw(tournament: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    if tournament.get("isLive") and current_url:
        for item in url_without_apostrophe_variants(current_url):
            add_url_candidate(candidates, item)

    for item in url_without_apostrophe_variants(draws_url_raw):
        add_url_candidate(candidates, item)
    ...
    for candidate in candidates:
        ...
        try:
            ...
            if os.environ.get("SAVE_DEBUG_HTML", "0") == "1" and not first_zero_debug_saved:
...
def add_url_candidate(candidates: List[str], value: Optional[str]) -> None:
    if value and value not in candidates:
        candidates.append(value)


def url_without_apostrophe_variants(url: Optional[str]) -> List[str]:
    if not url:
        return []
    values: List[str] = []
...
def fetch_tournament_results(tournament: Dict[str, Any]) -> Tuple[List[Dict[str, str]], List[Dict[str, Any]], Optional[str]]:
    ...
    # Najpierw warianty current i bez apostrofu, bo ATP/Cloudflare potrafi oddać pustą stronę
    # dla adresu zawierającego /'s-hertogenbosch.
    if tournament.get("isLive") and current_url:
        for item in url_without_apostrophe_variants(current_url):
            add_url_candidate(candidates, item)

    for item in url_without_apostrophe_variants(scores_url_raw):
        add_url_candidate(candidates, item)
    ...
    if draws_url:
        ...
        if tournament.get("isLive"):
            ...
            for item in url_without_apostrophe_variants(results_from_draw_current):
                add_url_candidate(candidates, item)

        for item in url_without_apostrophe_variants(results_from_draw):
            add_url_candidate(candidates, item)
...
def main() -> None:
    ...
    for index, tournament in enumerate(selected, start=1):
        ...
        if draw_source_url is not None:
            ...
            # Nie zapisujemy żadnych flag przy blokadzie, bo samo to robi niepotrzebny commit.
            draw_rounds = existing_draw_payload.get("rounds", []) or []
            draw_match_count = int(existing_draw_payload.get("countMatches") or 0)
        else:
            # Brak istniejących danych i brak źródła: nic nie zapisujemy, żeby nie tworzyć pustych plików.
            pass
        ...
        if source_url is None and isinstance(existing_matches_payload, dict):
            ...
            saved_matches_payload = matches_payload
        else:
